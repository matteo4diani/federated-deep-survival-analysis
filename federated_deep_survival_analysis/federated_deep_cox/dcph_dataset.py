...
def prepare_torch_dataset(
    num_partitions: int, batch_size: int, val_ratio: float = 0.1, random_seed=0
):
    ...
    num_examples = len(trainset) // num_partitions

    # a list of partition lenghts (all partitions are of equal size)
    partition_len = [num_examples] * num_partitions
    ...
    trainsets = random_split(
        trainset, partition_len, torch.Generator().manual_seed(random_seed)
    )
