...
def main(config: DictConfig):
    ...
    strategy = fl.server.strategy.FedAvg(
        fraction_fit=sys.float_info.min,  # in simulation, since all clients are available at all times, we can just use `min_fit_clients` to control exactly how many clients we want to involve during fit
        min_fit_clients=config.num_clients_per_round_fit,  # number of clients to sample for fit()
        fraction_evaluate=sys.float_info.min,  # similar to fraction_fit, we don't need to use this argument.
        min_evaluate_clients=config.num_clients_per_round_eval,  # number of clients to sample for evaluate()
        min_available_clients=config.num_clients,  # total clients in the simulation
